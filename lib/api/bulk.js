...
Job.prototype.retrieveBatches = async function(callback) {
  var self = this;
  return self.list().then( async (batchInfoList) => {

    // Get batches to load
    let batches = batchInfoList.map(function(batchInfo) {

      // NOTE: 'NotProcessed' is for the first batch when pkchunking
      if (batchInfo.state === 'NotProcessed' || parseInt(batchInfo.numberRecordsProcessed, 10) === 0) {
        return null;
      };
    ...
    .map(id => self.batch(id).retrieve()) // load all batches

    let batchResults = await Promise.all(batches);
    const results = [];
    batchResults.forEach(function(batchResult) {
      if (!batchResult) return;
      results.push(...batchResult); // concat into results
    });
  ...
  var poll = function() {
    ...
    job.info().then( async (jobInfo)=> {
      ...
      if (err) {
        ...
        // PK Chunking batches will be labeled 'NotProcessed'
        // https://developer.salesforce.com/docs/atlas.en-us.api_asynch.meta/api_asynch/async_api_headers_enable_pk_chunking.htm
        // success
        else if (res.state === "NotProcessed" && batchesComplete) {
          let results = await job.retrieveBatches();
...
Bulk.prototype.query = function(soql, options) {
  ...
  this.load(type, "query", options, soql)
  .then( async (results)=> {

    // Emit batches one at a time to avoid blowing up memory
    for (const result of results) {
      let records = await this.loadBatch(result);
      recordStream.emit('batch', records);
    }
...
// Load batch data
//
// TODO: Actually stream this? The problem is streaming multiple batches and parsing the csv.
// The csv module will parse the 2nd batch headers as a record.
Bulk.prototype.loadBatch = function(result) {
  const csvParser = CSV.parseCSVStream({ delimiter: ',', columns: true });
